...
public class RightsExportResult extends AbstractReadOnlyEntity {
    ...
    public static final String SEARCH_QUERY =
            ...
            + "(SELECT name_firstpart || '-' || name_lastpart FROM cadastre.cadastre_object WHERE id = baco.spatial_unit_id LIMIT 1) AS parcel_number, "
            + "(SELECT size FROM cadastre.spatial_value_area WHERE spatial_unit_id = baco.spatial_unit_id AND type_code='officialArea' LIMIT 1) AS area "
            ...
            + "	    WHERE pr.type_code = 'accountHolder' LIMIT 1 "
